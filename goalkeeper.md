...
The Law should probably read "outside the goalkeeper's penalty area" but is still correct. Read more in the [handball section](/handball).
...
[Indirect free kick. Player kicks ball while goalkeeper is releasing it. Also, generally a yellow card becuase of how blatant it is.](https://www.youtube.com/watch?v=fm0SBD5Pfd0)
...
"Goalkeeper backpass" is the unoffical, but commonly used term when the goalkeeper handles the ball when not supposed after passed from a team-mate.
...
If there is any doubt in the referee's mind, it should not be called. These would not be goalkeeper passback:
...
> [It is an indirect free kick offense/ caution for unsporting behvior if a player:]
> - initiates a deliberate trick for the ball to be passed (including from a free kick or goal kick) to the goalkeeper with the head, chest, knee etc. to circumvent the Law, whether or not the goalkeeper touches the ball with the hands; the goalkeeper is cautioned/penalised if responsible for initiating the deliberate trick
